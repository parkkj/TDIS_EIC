 ...
 * 
 * This code is for ep or eD : by change ABeam in "TDISMC_EICn.h" 
 * neutron tagging 
 */
// General definition in this header
#include "TDISMC_EICn.h"
#include <time.h>
...
// define f2N for neutron and proton
double  f2allm_f2d_q2dep(double x, double q2);
...
int mainx(double xMin,double xMax, double Q2Min,double Q2Max){
  ...
  //Int_t rnumber = gRandom->Uniform(10000); \\ big number in (<10000000000)
    ...
    double W2;
    int pr_particle_id,beam_particle_id;
    double prmass,MBeam;
    ...
    if(ABeam>1.){
      ...
      mPion = mPi0;  // pi0
      pi_particle_id = 111;
      pi_particle_charge=0; // pi0
      pr_particle_charge=1; // proton
      inucl = 1; // for proton in deuteron beam
      prmass =  MProton;
      pr_particle_id =  2212;
      beam_particle_id= 1000010020;
      MBeam = MDeut; 
    }
    else{
      ...
      mPion = mPip; // pi+
      pi_particle_id = 211;
      pi_particle_charge=1; // piplus
      pr_particle_charge=0; // neutron
      inucl = 1; // for neutron spectator from free proton beam
      prmass =  MNeutron;
      pr_particle_id =  2112;
      beam_particle_id= 2212;
      MBeam = MProton; 
    }
    int charge;
	...
	ofstream OUT ("TDISn_lund.txt", ios::app);
	...
	for (int iEvt=0; iEvt<NEvts; iEvt++) {
		...
		*/
		...
		if (TDIS_xbj > 0. && TDIS_xbj < 1.0){
		  ...
		  //f2N   = F2N(TDIS_xbj, invts.Q2, 2);  // F2N is define at G4SBSDIS.hh
		  ...
		  if(ABeam>1.){
		    ...
		    f2allmfac = f2allm_f2d_q2dep(TDIS_xbj, invts.Q2);
		  }
		  sigma_dis    = cdissigma_p(TDIS_xbj,invts.y_D,invts.Q2,invts.nu,kScattered_Rest.E()) *nanobarn * f2allmfac;
		...
		*/
		...
		//                    spectator is "neutron" 
		//
		// **********************************************************
		// Generate pSpectator Spherically Symmetric in rest frame : general Spectator Particle (slow Neutron)
		// **********************************************************
		//		psf     *= 4.*pi*pSMax^3/3;
		uu       = ran3.Uniform();
       ...
       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "2" << " \t " << d_particle_charge << " \t " << " 1 "<< " \t " << beam_particle_id << " \t " << "0" <<  " \t "<< "1" << " \t "<< scientific << PBeamMCx+PBeamMC*sin(CrossingTheta)*cos(CrossingPhi) << " \t " << PBeamMCy+PBeamMC*sin(CrossingTheta)*sin(CrossingPhi) << " \t " << PBeamMC*cos(CrossingTheta) << " \t " << PBeamMC << " \t " << MBeam << " \t " << vd0X_Lab  << " \t " << vd0Y_Lab << " \t " << vd0Z_Lab << endl; 
       ...
       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "5" << " \t " << pr_particle_charge << " \t " << " 1 "<< " \t " << pr_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific<< pprx_Lab << " \t " << ppry_Lab << " \t " << pprz_Lab << " \t " << EprE_Lab << " \t " << prmass << " \t " << vprx_Lab  << " \t " << vpry_Lab << " \t " << vprz_Lab << endl; 
       ...
       //       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "5" << " \t " << pi_particle_charge << " \t " << " 1 "<< " \t " << pi_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific<< ppix_Lab << " \t " << ppiy_Lab << " \t " << ppiz_Lab << " \t " << EpiE_Lab << " \t " << mPion << " \t " << vpix_Lab  << " \t " << vpiy_Lab << " \t " << vpiz_Lab << endl; 
       // the first spectator protn (TDIS)
       OUT << setiosflags(ios::left) << setiosflags(ios::fixed) <<  "\t" << "4" << " \t " << pr_particle_charge << " \t " << " 1 "<< " \t " << pr_particle_id << " \t " << "0" <<  " \t "<< "1" <<  " \t "<< scientific<< pprx_Lab << " \t " << ppry_Lab << " \t " << pprz_Lab << " \t " << EprE_Lab << " \t " << prmass << " \t " << vprx_Lab  << " \t " << vpry_Lab << " \t " << vprz_Lab << endl; 
